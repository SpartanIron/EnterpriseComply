...
async function migrate() {
  ...
  try {
    ...
    // notifications table — live event-driven notifications for in-app notification center
    ...
    await client.query(`ALTER TABLE notifications ADD COLUMN IF NOT EXISTS dedup_key TEXT`);
    await client.query(`ALTER TABLE notifications ADD COLUMN IF NOT EXISTS link TEXT`);
    await client.query(`CREATE INDEX IF NOT EXISTS notifications_org_id_idx ON notifications (org_id)`);
