...
export class Microsoft365Provider {
  ...
  async syncOrgMicrosoft365(orgId: number): Promise<{ collected: number; errors: string[] }> {
    const integration = await db.query.orgIntegrationsTable.findFirst({
      where: (t, { and }) => and(eq(t.orgId, orgId), eq(t.integrationKey, 'microsoft-365'), eq(t.status, 'connected'))
    });
    if (!integration?.config) return { collected: 0, errors: ['Microsoft 365 not connected'] };
    const config = (integration.config ?? {}) as unknown as Microsoft365Config;
    const errors: string[] = [];
    let collected = 0;
    try {
      ...
      if (authResp.ok) {
        const authData = await authResp.json() as any; // typed below
        const users = authData.value || [];
        const mfaRegistered = users.filter((u: any) => u.isMfaRegistered).length;
        await db.insert(orgEvidenceTable).values({ orgId, ucoControlId: 'UCO-AI-001', source: 'microsoft-365', title: 'Microsoft 365 MFA Registration', collectedAt: new Date(), description: `Microsoft 365: ${mfaRegistered}/${users.length} users MFA registered.`, metadata: { contentHash: '', totalUsers: users.length, mfaRegistered } });
        collected++;
      }
      const caResp = await fetch('https://graph.microsoft.com/v1.0/identity/conditionalAccess/policies?$top=50', { headers });
      if (caResp.ok) {
        const caData = await caResp.json() as any; // typed below
        const enabled = (caData.value || []).filter((p: any) => p.state === 'enabled').length;
        await db.insert(orgEvidenceTable).values({ orgId, ucoControlId: 'UCO-AI-002', source: 'microsoft-365', title: 'M365 Conditional Access Policies', collectedAt: new Date(), description: `Microsoft 365: ${enabled} Conditional Access policies enabled.`, metadata: { contentHash: '', enabledCAPolicies: enabled } });
        collected++;
      }
      const scoreResp = await fetch('https://graph.microsoft.com/v1.0/security/secureScores?$top=1', { headers });
      if (scoreResp.ok) {
        const scoreData = await scoreResp.json() as any; // typed below
        const score = scoreData.value?.[0];
        if (score) {
          await db.insert(orgEvidenceTable).values({ orgId, ucoControlId: 'UCO-CM-002', source: 'microsoft-365', title: 'Microsoft Secure Score', collectedAt: new Date(), description: `Microsoft Secure Score: ${score.currentScore}/${score.maxScore}`, metadata: { contentHash: '', secureScore: score.currentScore, maxScore: score.maxScore } });
          collected++;
        }
      }
