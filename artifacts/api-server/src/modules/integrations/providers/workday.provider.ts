...
export class WorkdayProvider {
  async syncOrgWorkday(orgId: number): Promise<{ collected: number; errors: string[] }> {
    const integration = await db.query.orgIntegrationsTable.findFirst({ where: (t, { and }) => and(eq(t.orgId, orgId), eq(t.integrationKey, 'workday'), eq(t.status, 'connected')) });
    if (!integration?.config) return { collected: 0, errors: ['Workday not connected'] };
    const config = (integration.config ?? {}) as unknown as WorkdayConfig;
    const errors: string[] = [];
    let collected = 0;
    try {
      ...
      if (workersResp.ok) {
        const workersData = await workersResp.json() as any; // typed below
        await db.insert(orgEvidenceTable).values({ orgId, ucoControlId: 'UCO-ST-001', source: 'workday', title: 'Workday Employee Roster', collectedAt: new Date(), description: `Workday HCM: ${workersData.total || 0} workers. Employee lifecycle tracking active.`, metadata: { contentHash: '', totalWorkers: workersData.total || 0 } });
        collected++;
      }
