...
const STATUS_ICONS: Record<string, React.ReactNode> = {
  ...
  inherited: <GitBranch className="w-3.5 h-3.5 text-blue-600" />,
  ...
  return (
    <div data-testid="controls-page" className="space-y-4">
      ...
      <div className="bg-card border border-border rounded-lg shadow-xs overflow-hidden">
        <div className="flex items-center gap-0 border-b border-border overflow-x-auto" data-testid="filter-bar">
          {STATUSES.map((s) => (
            <button
              key={s || "all"}
              ...
              <tbody className="divide-y divide-border">
                {(list.data?.controls ?? []).map((c) => (
                  <tr key={c.id} className="hover:bg-muted/20 transition-colors" data-testid={`control-row-${c.id}`}>
                    ...
                    <td className="py-3 px-4">
                      <div className="font-medium text-foreground max-w-[180px] truncate">{c.name}</div>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1.5">
                        {STATUS_ICONS[c.status]}
                        <span className={cn("inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide", STATUS_BADGE[c.status] ?? "")}>
                          {c.status}
                        </span>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        <div className="w-14 h-1.5 bg-muted rounded-full">
                          <div
                            className={cn("h-full rounded-full", c.effectiveness >= 80 ? "bg-green-500" : c.effectiveness >= 60 ? "bg-amber-500" : "bg-red-500")}
                            style={{ width: `${c.effectiveness}%` }}
                          />
                        </div>
                        <span className={cn("font-mono font-semibold text-xs", c.effectiveness >= 80 ? "text-green-600" : c.effectiveness >= 60 ? "text-amber-600" : "text-red-600")}>
                          {c.effectiveness.toFixed(0)}%
                        </span>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <span className={cn("inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide", SEV_BADGE[c.severity] ?? "")}>
                        {c.severity}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex flex-wrap gap-1 max-w-[160px]">
                        ...
                        {c.frameworks.length > 3 && <span className="text-[10px] text-muted-foreground font-medium">+{c.frameworks.length - 3}</span>}
                      </div>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4">
                      {c.driftDetected ? (
                        <div className="flex items-center gap-1 text-orange-600 font-medium text-[11px]">
                          <AlertTriangle className="w-3 h-3" />Drift
                        </div>
                      ) : <span className="text-muted-foreground text-sm">—</span>}
                    </td>
                    <td className="py-3 px-4">
                      <span className={cn("text-[11px] font-semibold", c.evidenceFresh ? "text-green-600" : "text-red-600")}>
