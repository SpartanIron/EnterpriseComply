...
const FISMA_METRICS = {
  fy2025: {
    ...
    domains: [
      ...
      { id: "data_mgmt", name: "Data Management", score: 85, weight: 11, grade: "B+", trend: "up", nistControls: ["SC-28", "MP-2", "MP-3"], status: "green", details: "Data classification applied to 92% of repositories. DLP policies active. Encryption at rest and in transit enforced." },
      { id: "network_apps", name: "Network & Application Security", score: 80, weight: 11, grade: "B", trend: "stable", nistControls: ["SC-7", "AC-4", "SI-3"], status: "yellow", details: "Zero trust network access deployed. Micro-segmentation in progress. 2 legacy perimeter firewall dependencies remain." },
      { id: "vuln_mgmt", name: "Vulnerability Management", score: 71, weight: 11, grade: "C+", trend: "down", nistControls: ["RA-5", "SI-2", "CM-6"], status: "yellow", details: "Critical vulns remediated within SLA: 67%. High vulns within SLA: 81%. Patch management gap on legacy systems." },
      { id: "training", name: "Security Awareness & Training", score: 94, weight: 11, grade: "A", trend: "up", nistControls: ["AT-2", "AT-3"], status: "green", details: "100% annual training completion. Phishing simulation pass rate 94%. Role-based training for ISSOs/ISSMs current." },
      { id: "conmon", name: "Continuous Monitoring", score: 88, weight: 11, grade: "B+", trend: "up", nistControls: ["CA-7", "SI-4", "AU-6"], status: "green", details: "ISCM program established. Automated evidence collection active. Monthly scan results submitted to CDM Dashboard." },
      { id: "incident_resp", name: "Incident Response", score: 82, weight: 11, grade: "B", trend: "stable", nistControls: ["IR-4", "IR-5", "IR-6"], status: "green", details: "IR plan tested Q3. CISA reporting within 1-hour threshold met. 3 major incidents reported and resolved this quarter." },
      { id: "contracts", name: "Contractor Systems", score: 76, weight: 11, grade: "B", trend: "stable", nistControls: ["SA-9", "SA-12"], status: "yellow", details: "93% of contractor systems with ATO or equivalent. 2 systems pending. DFARS compliance for all 800-171 contracts verified." },
    ],
  }
};
...
const POAM_SUMMARY = {
  ...
  criticalHigh: 8,
  scheduled30Days: 12,
  operationalRequirements: 3,
  riskAcceptances: 5,
  closedThisQuarter: 14,
};
...
const INCIDENT_SUMMARY = {
  ...
  category4: 3,
  cisaReported: 1,
  avgTimeToDetect: "4.2 hours",
  avgTimeToContain: "18.6 hours",
  avgTimeToRecover: "31.4 hours",
};
...
export default function FISMAReporting() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <select value={activeQuarter} onChange={e => setActiveQuarter(e.target.value)} className="px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <option value="fy2024q3">Q3 FY2024 (Apr-Jun 2024)</option>
          </select>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="text-left px-6 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider">NIST Controls</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider">Summary</th>
              </tr></thead>
              <tbody className="divide-y divide-slate-100">
                {metrics.domains.map(d => (
                  <tr key={d.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4"><GradeBadge grade={d.grade} /></td>
                    <td className="px-6 py-4"><div className="flex items-center gap-2"><StatusDot status={d.status} /><span className="text-xs font-medium capitalize" style={{ color: d.status === "green" ? "#22c55e" : d.status === "yellow" ? "#f59e0b" : "#ef4444" }}>{d.status}</span></div></td>
                    <td className="px-6 py-4"><div className="flex flex-wrap gap-1">{d.nistControls.map(c => <span key={c} className="text-xs font-mono px-1.5 py-0.5 rounded bg-blue-50 text-blue-700 border border-blue-100">{c}</span>)}</div></td>
                    <td className="px-6 py-4 text-xs text-slate-500 max-w-xs">{d.details}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeTab === "poam" && (
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          ...
          <div className="p-4 rounded-xl bg-blue-50 border border-blue-200">
            <p className="text-sm font-semibold text-blue-800 mb-2">FISMA POA&M Submission Requirements (OMB M-21-02)</p>
            <ul className="space-y-1 text-xs text-blue-700">
              <li>All High/Critical findings must have POA&M entries with documented remediation milestones</li>
              <li>POA&M entries must include original risk rating, residual risk, and scheduled completion date</li>
              <li>Risk acceptances require AO signature and 1-year maximum duration before re-evaluation</li>
              <li>Operational requirements must document the security impact and compensating controls</li>
            </ul>
          </div>
        </div>
      )}
      ...
      {activeTab === "incidents" && (
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <h3 className="text-sm font-bold text-slate-800 mb-4">US-CERT Incident Summary (FISMA Section 3546)</h3>
          ...
          <h4 className="text-sm font-semibold text-slate-700 mb-3">Incidents by US-CERT Category</h4>
          <div className="overflow-hidden rounded-xl border border-slate-200">
            <table className="w-full text-sm">
              <thead><tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider">Count</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider">US-CERT Report Req.</th>
              </tr></thead>
              <tbody className="divide-y divide-slate-100">
                {[
                  { cat: "CAT 1", desc: "Unauthorized Access", count: INCIDENT_SUMMARY.category1, required: "1 hour" },
                  { cat: "CAT 2", desc: "Denial of Service", count: INCIDENT_SUMMARY.category2, required: "2 hours" },
                  { cat: "CAT 3", desc: "Malicious Code", count: INCIDENT_SUMMARY.category3, required: "1 hour" },
                  { cat: "CAT 4", desc: "Improper Usage", count: INCIDENT_SUMMARY.category4, required: "1 hour (Major only)" },
                ].map(r => (
                  <tr key={r.cat} className="hover:bg-slate-50">
                    <td className="px-4 py-3 font-mono text-xs font-bold text-blue-700">{r.cat}</td>
                    <td className="px-4 py-3 text-slate-700">{r.desc}</td>
                    <td className="px-4 py-3 font-bold text-slate-800">{r.count}</td>
                    <td className="px-4 py-3 text-xs text-slate-500">{r.required}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeTab === "export" && (
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              { title: "FISMA Annual Report", desc: "Complete FY annual submission per OMB M-21-02. Includes all 9 CIO metrics domains, POA&M summary, incident counts, and ATO inventory.", format: "PDF + Excel", icon: "d" },
              { title: "FISMA Quarterly CIO Report", desc: "Quarterly metrics update for CIO dashboard submission. Formatted per CISA FISMA Metrics template.", format: "Excel (.xlsx)", icon: "d" },
              { title: "FITARA Scorecard Data", desc: "Data export for FITARA scorecard contribution. Maps cybersecurity metrics to FITARA evaluation criteria.", format: "Excel + JSON", icon: "d" },
              { title: "CDM Dashboard Export", desc: "DEFEND/MANAGE/PROTECT/CONNECT data package for submission to CISA CDM Dashboard.", format: "STIX 2.1 / JSON", icon: "d" },
              { title: "eMASS Data Package", desc: "Authorization package data formatted for import into eMASS system of record.", format: "XML (eMASS)", icon: "d" },
              { title: "OSCAL System Security Plan", desc: "Machine-readable SSP in NIST OSCAL JSON format. Compatible with FedRAMP OSCAL validation tooling.", format: "OSCAL JSON", icon: "d" },
            ].map(p => (
