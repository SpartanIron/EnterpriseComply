...
import { AlertTriangle, Target, ArrowRight, Zap } from "lucide-react";
...
function SevBadge({ sev }: { sev: string }) {
  return (
    <span className={cn("inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide", SEV_BADGE[sev] ?? "bg-slate-100 text-slate-600")}>
      {sev}
    </span>
  );
}
          ...
          {risks.isLoading ? (
            ...
            <div className="divide-y divide-border">
              {(risks.data?.risks ?? []).map((r) => (
                <div key={r.id} className="flex items-stretch" data-testid={`risk-item-${r.id}`}>
                  {/* Left-border accent */}
                  <div className="w-[3px] shrink-0" style={{ backgroundColor: SEV_ACCENT[r.severity] ?? "#cbd5e1" }} />
                  <div className="flex-1 p-4 hover:bg-muted/20 transition-colors">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 overflow-hidden">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {r.attackVectors.map((v) => (
                            <span key={v} className="rounded bg-orange-50 text-orange-700 ring-1 ring-inset ring-orange-200 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide">{v}</span>
                          ))}
                        </div>
                      </div>
                      <div className="shrink-0 text-right">
                        ...
                        <div className="text-[10px] text-muted-foreground mt-1 font-medium">
                          Blast radius: <span className="font-mono font-bold text-foreground">{r.blastRadius}</span>
                        </div>
                      </div>
                    </div>
                    <div className="mt-2.5 flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-medium text-muted-foreground">
                        Exploitability <span className="font-mono font-bold text-foreground">{(r.exploitability * 100).toFixed(0)}%</span>
                      </span>
                    </div>
                    {r.threatIntelligence && (
                      <div className="mt-2 inline-flex items-center gap-1.5 rounded bg-red-50 text-red-700 ring-1 ring-inset ring-red-200 px-2 py-1 text-[10px] font-bold">
          ...
          {attackPaths.isLoading ? (
            ...
            <div className="divide-y divide-border">
              {(attackPaths.data ?? []).map((ap) => (
                <div key={ap.id} className="flex items-stretch" data-testid={`attack-path-${ap.id}`}>
                  <div className="w-[3px] shrink-0" style={{ backgroundColor: SEV_ACCENT[ap.severity] ?? "#cbd5e1" }} />
                  <div className="flex-1 p-4 hover:bg-muted/20 transition-colors">
                    <div className="flex items-center justify-between mb-1.5">
                      <SevBadge sev={ap.severity} />
                      <div className="flex gap-4 text-[11px] font-medium text-muted-foreground">
                        <span>Likelihood <span className="font-mono font-bold text-foreground">{(ap.likelihood * 100).toFixed(0)}%</span></span>
                        <span>Impact <span className={cn("font-mono font-bold", ap.impact >= 0.9 ? "text-red-600" : "text-amber-600")}>{(ap.impact * 100).toFixed(0)}%</span></span>
                      </div>
                    </div>
                    <div className="text-sm font-bold text-foreground mb-2">{ap.name}</div>
                    ...
                    <div className="flex items-center gap-2 pt-2 border-t border-border text-[11px]">
                      ...
                      <div className="flex items-center gap-1 text-red-600"><AlertTriangle className="w-3 h-3" /><span className="font-bold">{ap.targetAsset}</span></div>
                    </div>
                  </div>
                </div>
              ))}
