...
/** Shape returned by GET /api/auth-providers */
...
export default function SignIn() {
  ...
  // P1-21: Fetch which OAuth providers are configured in this deployment.
  // Null = still loading; false/true per provider once resolved.
  const [providers, setProviders] = useState<ActiveProviders | null>(null);

  useEffect(() => {
    fetch(apiUrl("/auth-providers"))
      .then((r) => (r.ok ? r.json() : null))
      .catch(() => null)
      .then((data: ActiveProviders | null) =>
        // Safe fallback: if endpoint unreachable, assume no providers configured
        // rather than showing buttons that will fail.
        setProviders(data ?? { google: false, github: false }),
      );
  }, []);
  ...
  async function handleGoogle() {
    ...
    try {
      // P1-21: BetterAuth signIn.social returns { data, error } rather than always
      // throwing. Check the error property explicitly so silent failures are surfaced.
      const result = await authClient.signIn.social({ provider: "google", callbackURL: CALLBACK_URL });
      if ((result as any)?.error) {
        const msg = (result as any).error?.message;
        setError(msg ?? "Google sign-in is not available. Please use GitHub or magic link.");
        setGoogleLoading(false);
      }
      // On success the browser is redirected — nothing to do here.
    } catch (err: any) {
  ...
  async function handleGitHub() {
    ...
    try {
      await authClient.signIn.social({ provider: "github", callbackURL: CALLBACK_URL });
      // Note: on success, browser is redirected to GitHub — this line typically never runs
    } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center" style={{ background: "#f1f5f9" }}>
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          ...
          {/* OAuth buttons — conditionally rendered based on /api/auth-providers response.
              P1-21: Google button is hidden (not just disabled) when GOOGLE_CLIENT_ID/SECRET
              are absent from the deployment env. Prevents silent OAuth failures. */}
          <div className="px-7 mb-3 space-y-2.5">
            {/* Google — only rendered when providers.google === true */}
            {providers?.google && (
              <button
                type="button"
            ...
            {/* GitHub — only rendered when providers.github === true */}
            {/* P1-11: GitHub OAuth — works without SMTP, usable in dev/staging immediately.
                Credentials (GITHUB_CLIENT_ID / GITHUB_CLIENT_SECRET) are set in env.
                This is the primary non-email login path until SMTP/Resend is configured. */}
            {providers?.github && (
              <button
                type="button"
