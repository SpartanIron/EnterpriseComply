...
export default function SignIn() {
  ...
  async function handleGoogle() {
    ...
    try {
      await authClient.signIn.social({ provider: "google", callbackURL: BASE_PATH + "/" });
    } catch {
  ...
  async function handleMagicLink(e: React.FormEvent) {
    ...
    try {
      if (tab === "signup" && name.trim()) {
        await authClient.signUp.email({ name: name.trim(), email, password: crypto.randomUUID(), callbackURL: BASE_PATH + "/" });
      }
      const result = await authClient.signIn.magicLink({ email, callbackURL: BASE_PATH + "/" });
      if (result?.error) {
