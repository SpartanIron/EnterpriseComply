...
- Task 1.6 (CMMC L1 blueprint) asked whether `RequirePlan("federal")` could exclude cmmc-l1 by being framework-aware rather than plan-tier-aware. It is plan-tier only: `PLAN_HIERARCHY` and `PlanGuard.canActivate` read only `org.plan`, with no framework field anywhere in the guard. `present:file:artifacts/api-server/src/guards/plan.guard.ts` (no framework field referenced in that file's guard logic). ColorCode Solutions' org is already on the federal plan (verified live at /settings), so this does not block cmmc-l1 SSP/POA&M/SPRS access once the framework itself is visible.
- Task 1.3 (partial): the corp-email dashboard checklist item cited the legacy CMMC 1.0 ID `IA.3.058`; normalized to the CMMC 2.0 ID `IA.L1-3.5.1` (a Level 1 practice). `present:file:artifacts/c2s-ciop/src/pages/Dashboard.tsx`. The other legacy IDs found in this file (`IA.3.083`, `AC.2.006`, `CA.2.157`) are Level 2 identifiers whose in-context usage does not cleanly match their textbook 1.0-to-2.0 crosswalk meaning; left as-is and flagged for a human compliance review rather than guessed at.
  - Task 1.4: added the two UCO controls the blueprint identified as missing (`UCO-DP-007`, `UCO-PS-006`) and populated `frameworkKey="cmmc-l1"` rows in `ucoFrameworkMappingsTable` for all 17 CMMC Level 1 practices. `present:file:lib/db/src/seed-colorcomply.ts`. Correction to the blueprint's own premise: `control_crosswalk` (`lib/db/src/schema/orgCrosswalk.ts`) is dead code with 0 rows -- its own read endpoint returns `[]` unconditionally per the header comment in `artifacts/api-server/src/lib/framework-mappings.ts` -- so this PR populates the table that is actually read (`ucoFrameworkMappingsTable`), not `control_crosswalk`. `present:file:artifacts/api-server/src/lib/framework-mappings.ts`.
    - Task 1.5: added a config-driven admin-email-domain guardrail. `present:file:artifacts/api-server/src/lib/admin-email-policy.ts`, wired into invite creation at `present:file:artifacts/api-server/src/modules/orgs/invites.service.ts`. Deny-list defaults to the same personal-domain set already used cosmetically in Dashboard.tsx, and is overridable via the `BLOCKED_ADMIN_EMAIL_DOMAINS` env var. Deliberately not a per-org DB-backed setting: `organizations` has no generic settings column (`present:file:lib/db/src/schema/organizations.ts`) and `drizzle-kit push` is a manual, human-run step in this repo (`present:file:lib/db/package.json`), not something this session could safely apply to production. Only blocks new admin invitations; does not touch any existing account.
