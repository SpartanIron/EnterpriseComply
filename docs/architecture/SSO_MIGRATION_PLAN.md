# C2S Intel â Clerk Migration Plan (Path 1)
...
> **Effort estimate.** 2â5 focused engineering days.
> **Blast radius.** P0 â production auth change. Stage on Railway preview before production cutover.
...
| Railway preview environment exists for c2s-api-server | Service Settings â Environments shows a non-production env, or a feature-branch deploy is producing PR previews | Railway UI. |
...
## 1. Repo audit â what currently does auth
...
1. In Clerk dashboard â Configure â API Keys, confirm the production publishable key is `pk_live_Y2xlcmsuY29sb3Jjb2Rlc29sdXRpb25zLmNvbSQ` (same as EnterpriseComply â single instance, single key).
2. Copy the `sk_live_*` production secret key. **Owner only â never share.**
3. In Clerk dashboard â Domains, add allowed origins for C2S Intel:
...
### 5c. Locked decision (canonical)

> **Decision.** **5a (Clerk Import API)** is selected as the canonical migration strategy.
> **Decided by.** Account owner direction ("go with your recommendation"), 2026-05-18.
> **Rationale.** 5a is the only strategy that delivers zero customer-facing disruption — existing C2S Intel users sign in post-cutover with their existing Google identity and land in their existing account with no re-onboarding step, no "why am I being asked to sign in again" moment, and no edge-case handling for users whose email may have drifted from their google_id. For a federal-BD-intelligence product where customer trust and continuity matter more than a few extra hours of engineering work, the higher engineering cost of 5a is the correct trade.
> **Conditional fallback to 5b.** If, during Section 1 (repo audit), the active user count is discovered to be **fewer than 10**, and the owner explicitly approves the simpler path in writing, the engineer MAY fall back to 5b. Below 10 users, the re-onboarding friction is acceptable and the Clerk Import API setup overhead is not justified. Any fallback to 5b requires updating this document and recording the approval in the cutover paper trail.
> **Owner-only step in 5a.** The actual POST to https://api.clerk.com/v1/users carrying the `sk_live_*` secret key is executed by the account owner only. The engineer prepares the JSON payload, the owner runs the curl/script.
> **Pre-import dry run.** Before the production import, the owner runs the same payload against a Clerk **test instance** (or a small subset of 1-2 users in production) to verify the JSON shape and the email-collision behavior. Only after the dry run succeeds does the full batch run.

---

## 6. Cutover sequence (production)

**Pre-cutover:** All Section 3, 4, 5 work merged to `main` on the C2S Intel repo. Railway preview deploy has been verified end-to-end: sign-in via Clerk works, protected routes work, user data is correctly linked to Clerk IDs, the launcher cookie is recognized (test by signing into `app.colorcodesolutions.com` then visiting the preview URL â should appear signed-in).
...
6. Smoke test:
   - Hit `https://c2s-api-server-production.up.railway.app` â loads.
   - Sign in via Google through Clerk â success.
   - Existing user lands in their data (Section 5a migration verified) OR new `clerk_user_id` written on first login (Section 5b).
   - Sign out â session ends.
   - Re-sign-in â session restored.
7. Watch Railway logs for 60 minutes. Any 401/403 spikes or stack traces with `session` or `passport` references â rollback (Section 7).
...
1. In Railway â c2s-api-server â Settings â Networking â Custom Domains: click "Add Custom Domain", enter `govcon.colorcodesolutions.com`. Railway provides a CNAME target.
2. In Cloudflare â colorcodesolutions.com â DNS: create CNAME `govcon` â `<railway-cname-target>`, proxied (orange cloud).
...
### 8b. Flip the launcher card COMING SOON â LIVE

The app-launcher Worker source is not in the EnterpriseComply repo. Engineer must either locate the launcher source repo OR edit the Worker directly in the Cloudflare dashboard (with the same change-management discipline as the clerk-proxy deploy â only when Cloudflare dashboard is healthy).
...
- [ ] Clicking the EnterpriseComply tile lands at `https://grc.colorcodesolutions.com` already authenticated â no second login.
- [ ] Clicking the C2S Intel tile lands at `https://govcon.colorcodesolutions.com` already authenticated â no second login.
- [ ] Signing out on the launcher signs the user out of both products.
- [ ] Browser DevTools â Application â Cookies on `app.colorcodesolutions.com` shows a Clerk session cookie scoped to `.colorcodesolutions.com`.
...
- [ ] Clerk dashboard â Users shows ONE record per real user across both products.
...
- Cloudflare dashboard must be healthy (GCR â¥ 80/100) before any Worker route changes in Section 8.
